...
def random_forest_regressor(train_x, train_y, n_estimators,  max_depth, random_state,n_jobs ):
	...
	model = RandomForestRegressor(n_estimators=n_estimators,max_features='sqrt',max_depth=max_depth,random_state=random_state ,n_jobs=n_jobs).fit(train_x, train_y)  # 
	print 'training took %fs!' % (time.time() - start_time)
	return model
